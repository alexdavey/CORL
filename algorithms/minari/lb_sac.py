...
def train(config: TrainConfig):
    ...
    eval_env = wrap_env(minari_dataset.recover_environment(eval_env=True))
    state_dim = eval_env.observation_space.shape[0]
    action_dim = eval_env.action_space.shape[0]

    d4rl_dataset = qlearning_dataset(minari_dataset)
    ...
    buffer.load_dataset(d4rl_dataset)
    ...
    for epoch in trange(config.num_epochs, desc="Training"):
        ...
        if epoch % config.eval_every == 0 or epoch == config.num_epochs - 1:
            ...
            with contextlib.suppress(ValueError):
                normalized_score = (
                    minari.get_normalized_score(minari_dataset, eval_returns) * 100.0
                )
