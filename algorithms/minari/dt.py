...
class TrainConfig:
    ...
    env_name: str = "D4RL/pen/human-v2"
...
def load_trajectories(
    ...
    obs, actions, rewards, timeouts, terminals = [], [], [], [], []
...
class SequenceDataset(IterableDataset):
    def __init__(
        ...
        self.dataset, info = load_trajectories(minari_dataset, gamma=1.0)
...
def eval_rollout(
    ...
    states[:, 0] = torch.as_tensor(env.reset(seed=seed)[0], device=device)
    ...
    for step in range(model.episode_len):
        ...
        next_state, reward, term, trunc, _ = env.step(predicted_action)
...
def train(config: TrainConfig):
    ...
    dataset = SequenceDataset(
        minari_dataset, seq_len=config.seq_len, reward_scale=config.reward_scale
    )
        ...
        if step % config.eval_every == 0 or step == config.update_steps - 1:
            ...
            for target_return in config.target_returns:
                ...
                with contextlib.suppress(ValueError):
                    normalized_scores = (
                        minari.get_normalized_score(minari_dataset, np.array(eval_returns)) * 100
                    )
                    wandb.log(
                        {
