...
class Config:
    ...
    # training params
    dataset_name: str = "D4RL/pen/human-v2"
    batch_size: int = 1024
    num_epochs: int = 1000
...
def qlearning_dataset(dataset: minari.MinariDataset) -> Dict[str, np.ndarray]:
    obs, next_obs, actions, next_actions, rewards, dones = [], [], [], [], [], []

    for episode in dataset:
        obs.append(episode.observations[:-1].astype(np.float32))
        next_obs.append(episode.observations[1:].astype(np.float32))
        actions.append(episode.actions[:-1].astype(np.float32))
        next_actions.append(episode.actions[1:].astype(np.float32))
        rewards.append(episode.rewards[:-1])
        dones.append(episode.terminations[:-1])
...
class ReplayBuffer:
    ...
    def create_from_d4rl(
        ...
        d4rl_data = qlearning_dataset(minari_dataset)
        buffer = {
        ...
        if normalize_reward:
            buffer["rewards"] = ReplayBuffer.normalize_reward(
                minari_dataset.spec.dataset_id, buffer["rewards"]
            )
        self.data = buffer
...
def train(config: Config):
    ...
    minari_dataset = minari.load_dataset(config.dataset_name, download=True)
    buffer = ReplayBuffer()
    buffer.create_from_d4rl(
        minari_dataset, config.normalize_reward, config.normalize_states
    )

    key = jax.random.PRNGKey(seed=config.train_seed)
    key, actor_key, critic_key = jax.random.split(key, 3)

    eval_env = make_env(minari_dataset, seed=config.eval_seed)
    ...
    for epoch in trange(config.num_epochs, desc="ReBRAC Epochs"):
        ...
        if epoch % config.eval_every == 0 or epoch == config.num_epochs - 1:
            ...
            with contextlib.suppress(ValueError):
                normalized_score = (
                    minari.get_normalized_score(minari_dataset, eval_returns) * 100
                )
                wandb.log(
                    {
